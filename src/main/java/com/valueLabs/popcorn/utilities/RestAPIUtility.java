    ...
    public void iValidateResponseBodyWithKey(String key, String expValueToValidate) throws Exception {
        if (responseAsString.isEmpty()) {
            ...
            if(expValueToValidate.contains("|")){
                ...
                if(ar.size()>0){
                    Assert.fail("FAILED, EXPECTED : "+Arrays.asList(expValueToValidate.replace("|",", "))+" ACTUAL is : "+ar);
                }else{
                    System.out.println("PASSED, EXPECTED : "+Arrays.asList(expValueToValidate)+" ACTUAL is : "+ar);
                }
